...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for olduser in orm.OldUser.objects.all():
            user = orm.User()
            for field in olduser._meta.fields:
                #omit, these will be set from olduser.user
                if field.name in ('_username', '_first_name', '_last_name', '_email'):
                    continue
                #handle user
                if field.name == 'user':
                    u = getattr(olduser, field.name)
                    for uf in u._meta.fields:
                        value = getattr(u, uf.name)
                        setattr(user, uf.name, value)
                #handle other fields
                else:
                    value = getattr(olduser, field.name)
